...
#############################################################################
## Name:        pipe.pm
## Purpose:     Statistics::Rpad::Bridge::pipe
...
{ package Statistics::Rpad::Bridge::pipe ;
  ...
  sub save_file_startR { 
    ...
    print $fh qq`
      ...
      cat( Sys.getpid() , "\\n" , file=.PERLPIDFILE)
      close(.PERLPIDFILE) ;
      if (!exists("guiSource")) guiSource <- function(file) source(file,print.eval=TRUE)  
      ...
      while(1) {
        ...
        cat("...\\n" , file=.PERLOUTPUTFILE)
        tryCatch( guiSource(.PERLINPUTFILE) , error = function(e) { print(e) } ) ;
...
Statistics::Rpad::Bridge::pipe - Base class for pipe communication with R.
