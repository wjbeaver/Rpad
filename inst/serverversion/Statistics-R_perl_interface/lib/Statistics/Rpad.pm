...
{ package Statistics::Rpad ;
  ...
  sub R { 
    ...
    $this->{BRIDGE} = Statistics::Rpad::Bridge->new(%args) || return UNDEF ;
  }
...
Statistics::Rpad - Controls the R (R-project) interpreter through Perl.
...
When creating the R bridje object (Statistics::Rpad), you can set some options:
...
To install this package you need to install R in your OS first, since I<Statistics::Rpad> need to find R path to work fine.

A standart installation of R on Win32 and Linux will work fine and detected automatically by I<Statistics::Rpad>.
...
The main pourpose of I<Statistics::Rpad> is to start a single R interpreter that hear
multiple Perl process.

Note that to do that R and Perl need to be running with the same user/group level.

To start the I<Statistics::Rpad> bridge you can use the script I<statistics-r.pl>:
