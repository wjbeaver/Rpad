...
HTML.guiInput <- function(p, varname) {
  cat("<td>",p$name,"</td><td>")
  print(HTMLinput(id=varname, name=varname, value= p$default, rpadtype=p$type,
...
HTML.guiCombobox <- function(p, varname) {
  ...
  print(HTMLinput(id=varname, name=varname, value= p$default, rpadtype=p$type, contenteditablewrapper=FALSE,
...
# write the hidden values
  # varname <- list()
  print(HTMLinput(name=varname, value="list()", type="hidden"))
  ...
  # idx <- 0
  print(HTMLinput(name=idx, value=0, type="hidden"))
  cat("<span name='cmd_wrapper'><table>")
  # idx <- idx + 1
  print(HTMLinput(name=idx, value=paste(idx,"+ 1"), type="hidden"))
  # varname[[idx]] = list()
  print(HTMLinput(name=paste(varname,'[[',idx,']]',sep=""),
                  value="list()", type="hidden"))
